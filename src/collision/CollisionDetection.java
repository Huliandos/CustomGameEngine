...
public class CollisionDetection {
	...
	public static boolean scanForCollision(GameObject go1, float posX, float posY) {
		if(go1.getCollider().getClass() == CircleCollider.class) {
			return Math.pow((posX - go1.getXPosition()), 2) + Math.pow((posY - go1.getYPosition()), 2) < Math.pow(((Zombie)go1).getRadius(), 2);
		}
		//TODO add behavior for player and Rectangles (maybe)
		return false;
	}
	...
	public static float getRaycastDistance(float x1, float y1, float x2, float y2, GameObject go) {
		float posX = 0, posY = 0, width = 0, height = 0;
		...
		return lineLineDistance(x1, y1, x2, y2, posX, posY, width, height);
	}
	
	// Line to line collision
		static float lineLineDistance(float lineX1, float lineY1, float lineX2, float lineY2, float posX, float posY, float width, float height) {
			...
			// if uA and uB are between 0-1, lines are colliding
			if (uA >= 0 && uA <= 1 && uB >= 0 && uB <= 1) {
				float collisionPointX = lineX1 + (uB * (rectX2-rectX1));
				float collisionPointY = lineX2 + (uB * (rectY2-rectY1));
				
				float potentialShortestDistance = (float) Math.sqrt(Math.pow(lineX1 - collisionPointX, 2) + Math.pow(lineY1 - collisionPointY, 2));
				System.out.println("Potential shortest distance: " + potentialShortestDistance);
				
				if(potentialShortestDistance < distance) distance = potentialShortestDistance;
			}
